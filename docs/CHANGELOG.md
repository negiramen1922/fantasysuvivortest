...
## ver.α1.0.159
- 無限ランの敵スケーリングを刷新（`makeEnemy`）。従来は `loopMul=1+eLoop*0.6`（15分ごと=フルループ単位）で、ステージが5分で循環するため弱ステージへ戻ると体感が下がった。→ **5分ブロック単位**の `blkMul=1+eBlk*0.5`（ブロック1=×1.5・2=×2.0…）に変更し `scale=(1+time/60*0.63)*blkMul`。加えて **HP下限** `hpFloor=140*eBlk*(1+time/60*0.4)` を導入し、最弱ザコもブロックごとに右肩上がり。`dscale` も `(1+eBlk*0.12)` に。
- スコア倍率：敵に `smul=(ENDLESS?blkMul:1)` を付与し、`score+=Math.round((...)*(e.smul||1))`。周回でHPが上がっても1体あたりスコアが上がるためスコアが伸びやすい（図鑑BSCOREは基準ptsのまま＝メタ進行は不変）。ボスは `smul` 未設定＝×1。検証：`check.py` OK＋ロード無エラー。
...
## ver.α1.0.153
- ボス討伐スコア閾値の調整（実測フィードバック反映）。実データ（ステージ1・強化ほぼ無し）で ②スライムキングが 9:19/SCORE44,327 で出現＝時間より約41秒早い＝理想。一方 ③④は 84k/140k だと弱ビルドでも4〜6分早く出る恐れ（スコアは討伐“数”でも伸びるため火力と乖離）。①② は据え置き、③④のみ約1.4倍へ：BOSS_AT ③84,000→120,000／④140,000→220,000、BOSS_AT2 ③104,000→146,000／④172,000→240,000、BOSS_AT3 ③128,000→180,000／④210,000→290,000。

## ver.α1.0.152
- ボス出現を「時間 or 討伐スコア の早い方」に変更（ぐだり防止）。`BOSS_AT`/`BOSS_AT2`/`BOSS_AT3` の各要素を `[time, key, score]` に拡張。`checkBoss` を `if(time<b[0] && score<(b[2]||Infinity)) return;` に変更し、いずれか到達で `spawnBoss`。あわせて `if(boss&&!boss.dead) return;` を追加し、前ボス健在中の重複出現（スコア前倒しでの多重湧き）を防止。
- スコア閾値はステージ別（後半ほどスコアの上がりが速いため高め）：S1=[16000,44000,84000,140000] / S2=[20000,54000,104000,172000] / S3=[26000,68000,128000,210000]。**暫定値・要調整**（ラン中の `score` は討伐pts＋反応×30＋銀貨×15の累積で、時間/レベル/クリアボーナスは終了時加算なので閾値には含まれない）。無限ラン(`checkBossEndless`)は従来どおり時間ベースで据え置き。
...
- 立ち絵は未登録のため `CHAR_SPR` には追加せず、当面はグリフ(🥷)／簡易円で表示。PNG アップ後に `CHAR_SPR.ninja` を追加予定。

## ver.α1.0.142
- 手裏剣の特別強化 `sh_bl` の名称を「裂傷」→「深手」に統一（刀の `kt_bl` と同名に）。id・効果(`ef:{bleed:.40}`)は据え置き。

## ver.α1.0.141
- チェンソーの点滅（チカチカ）を解消し、稼働/収納のデューティサイクル動作に変更。`runWeapons`: `P.sawOn`/`P.sawPh`(稼働2.6秒・収納1.4秒)を導入。取得直後は稼働から開始。稼働中は単一の長寿命 fx(`P.sawFx`, `sustain:1`)を毎フレーム更新(`t=0`で失効させない)して点滅を防止、ダメージは `doChainHit(rev,fa,arc,rad)` を `ready()` インターバルで呼ぶ形に分離。収納時は `P.sawFx.life=0` で消す。
- `drawSaw`: `f.sustain` のときはアルファをフェードさせず一定(0.9)に。旧 `doChain2`(毎回 fx を push する短命エフェクト)は廃止。

## ver.α1.0.140
- チェンソーが大きすぎたので縮小。`doChain2`: 基準半径 `rad` 95→62、扇 `Math.min(1.2,0.7*ar)`→`Math.min(1.0,0.6*ar)`。見た目(`drawSaw`)は `f.rad` 依存なので自動的に一回り小さくなる。

## ver.α1.0.139
- チェンソー描画を刷新。旧: 扇状に歯を10本並べる `drawSaw` → 新: 丸い先端のガイドバー（round-cap の太線カプセル＋縁を流れるチェーンの歯、覚醒時は全方位の丸ノコ）。当たり判定の扇に太さを合わせた（`w=rad*sin(arc/2)`）。
- チェンソーの向きを P.face（自動狙い）→ **移動方向** に。`P.mvdir`(移動時に記録) を導入、`arcHit(...,face)` にオプション引数を追加、`doChain2` で `fa=P.mvdir` を使用。扇も `Math.min(1.2,0.7*ar)` に狭めてバー状に。
- リザルトの与ダメージ内訳を拡張。出血ぶんを `WDMG._bleed` に分離（武器クレジットから減算し二重計上を回避）→「🩸出血」行を表示。元素反応の種類別 `REACTDMG`（`RK`=現在の反応名、`RNAME` で key→名称）を集計し、「元素反応の内訳」セクションで反応別に表示。
- 毒(草)の持続を 4→5 秒に延長（沼外でも継続することを明確化。DoTティックは元々毎フレーム無条件）。
- 検証：docs/check.py 通過。チェンソーが丸バーで移動方向へ、リザルトに🩸出血38(3%)表示、エラー無しを確認。
...
- 手裏剣の初期貫通を 2→0 に（`pierce:PRC('shuriken')`）。無強化で1体ヒット、貫通軸/熟練で増加。
- 出血ダメージ算出（参考）：ヒットダメージ×係数を「1秒あたり」として duration 継続。刀=hit×0.30×(1+TR bleed)/3秒、手裏剣=hit×0.50×(1+TR bleed)×(覚醒1.8)/2.5秒。`dot()` は Math.max で最強値に更新（非スタック）、防御無視・会心なし。
- 検証：docs/check.py 通過。ステージ3でカウンターが周囲弾を消去、刀=斬撃+出血、エラー無しを確認。
...
## ver.α1.0.121
- スケルトンドラゴン(sdragon)の追尾ブレス(beam)の旋回速度を `1.05rad/s`→`0.45rad/s` に緩和（追尾が強すぎるとのフィードバック対応）。ブレス継続1.9秒で最大約49°しか曲がらず、横に走れば回避しやすい。判定・太さ・ダメージは据え置き。

## ver.α1.0.120
- 武器熟練画面のヘッダー右上に「？」ヘルプボタン(`#masthelp`)と説明パネル(`#masthelpbox`)を追加。クリックでトグル表示。説明内容：熟練度は武器で敵を攻撃すると貯まる／同カテゴリ武器へ `SPILL_SAME=10%`・入手済みの他武器へ `SPILL_OTHER=5%` 波及／レベルアップで強化を1つ選べる／Lv3・6・9(major)は強力な強化／最大10レベル。`mastbtn` を開くたびにパネルは閉じた状態にリセット。検証：`?dev` で ? ボタン表示・トグル開閉・文言・エラー無しを確認。
